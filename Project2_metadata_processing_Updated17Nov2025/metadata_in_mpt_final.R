...
#since score of >=13 is depressed in primary care situations
metadata_dep_cat <- metadata %>%
  mutate(dep_cat = case_when(
    bdi_ii < 13 ~ "not depressed",
    bdi_ii >= 13 ~ "depressed"
  ))
...
sum(meta_novir$dep_cat == "depressed")       #BDI 10 64, 20 99
sum(meta_novir$dep_cat == "not depressed")   #BDI 10 92, 20 57
...
#count how many have depression and hypertension in each category
table(meta_subset$dep_hyp) 

sum(meta_subset$sex == "male") #93
sum(meta_subset$sex == "female") #62

meta_final = meta_subset %>%
  filter(meta_subset$dep_cat != "NA" & meta_subset$hypertension != "NA" & meta_subset$sex != "not provided" & meta_subset$host_age != "NA") %>% #remove all samples with NAs in sex, age, hypertension and depression
...
#view groups
table(meta_final$age_cat) #53 60 and older, 80 under 60
table(meta_final$dep_hyp_sex)
table(meta_final$dep_hyp_age)
